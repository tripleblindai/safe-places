...
function doXHR(method, url, param, auth, creds) {
	...
	return new Promise(function (resolve, reject) {
		...
		if (method === "GET" && isDefined(param)) {
			var respType = param;
			if (respType === "blob" || respType === "arrayBuffer" || respType === "document" ||
				respType === "json" || respType === "text")
...
if (!isInitalized(MAP_API_KEY) || MAP_API_KEY == "" || MAP_API_KEY == null ||
	MAP_API_KEY == "null" || MAP_API_KEY == "MAP_API_KEY") {
	...
	if (isInitalized(selectedArea)) {
		if (isInitalized(exposurePoints)) {
			...
			exposurePoints.forEach(function (element, index) {
				if (isInitalized(element.getMap()) && areaBounds.contains(element
					.getPosition())) {
		...
		fr.onload = (function (map, points, path) {
			return function (event) {
				...
				exposureJSON.forEach(function (element, index) {
					...
					if (!currentGroupBounds.isEmpty() && (google.maps.geometry
						.spherical
						.computeDistanceBetween(elementLatLng,
							currentGroupBounds
								.getCenter()) > 30.48)) {
					...
					if (exposureGroups[currentGroupId] === undefined ||
						exposureGroups[
						currentGroupId] === null) {
					...
					google.maps.event.addListener(marker, 'click', (function (
						...
						return function (event) {
							...
							thisMarker.setIcon(MARKER_ICONS
								.SELECTED);
							thisMarker.setZIndex(MARKER_ZINDEX
								.SELECTED);

							let uncolorColsure = (function (
								groupId) {
								return function (event) {
									exposureGroups[
										groupId]
							...
							enableDelete(
								exposureGroups[groupId].markers
									.length >
								1,
								true);
						}
				...
				let addTravelGroupTimeAndDistance = function (fromGroup, toGroup) {
					...
					let currentGroupEndTime = toGroup.elements[toGroup.elements.length -
						1]
						.time;

					if (isInitalized(fromGroup)) {
						fromGroupEndTime = fromGroup.elements[fromGroup.elements
							.length - 1]
							.time;
					...
					if (isInitalized(fromGroup)) {
						let fromGroupEndPosition = fromGroup.markers[fromGroup.markers
							.length -
							1].getPosition();
						...
						distance += google.maps.geometry.spherical
							.computeDistanceBetween(
								fromGroupEndPosition, toGroupStartPosition);
					}
					...
					toGroup.markers.forEach(function (element, index) {
						if (index > 0) {
							distance += google.maps.geometry.spherical
								.computeDistanceBetween(toGroup.markers[index -
									1]
				...
				let updateTravelGroupMarkers = function (travelGroupMarkers, travelGroupId,
					travelGroupDurationMilliseconds, travelGroupDistanceMeters) {
					let travelGroupDurationHours = travelGroupDurationMilliseconds / (
					...
					if (travelGroupDurationHours > 0) {
						travelGroupSpeedKMPH = travelGroupDistanceKilometers /
							travelGroupDurationHours;
					}

					//round to 2 decimal places...soooo ugly in javascript
					travelGroupSpeedMPH = Math.round(((travelGroupSpeedKMPH / 1.609) +
						Number.EPSILON) * 100) / 100;
					travelGroupSpeedKMPH = Math.round((travelGroupSpeedKMPH +
						Number.EPSILON) * 100) / 100;
				...
				for (const key of exposureGroupKeys) {
					...
					for (var i = groupCount; i < exposureGroups.length; i++) {
						...
						if (exposureGroups[key].bounds.intersects(exposureGroups[
							exposureGroupKeys[
							i]].bounds)) {
					...
					if (!foundIntersection && exposureGroups[key].elements[0].groupType !=
						...
						if (exposureGroups[key].elements.length <= travelMaxGroupSize) {
							...
							if (groupCount == 1 || exposureGroups[exposureGroupKeys[
								groupCount - 2]]
								.elements[0]
							...
							addTravelGroupTimeAndDistance(lastExposureGroup, exposureGroups[
								key]);
					...
					if (
						...
						updateTravelGroupMarkers(travelGroupMarkers, travelGroupId,
							travelGroupDurationMilliseconds, travelGroupDistanceMeters);
...
function initDateSlider(msStartDate, msEndDate) {
	...
	let sliderStepCount = Math.ceil((msEndDate - msStartDate) / incrementFactor) +
		24; //number of days to have the slider cover (has to be +1 because we take the end date and move back to midnight)
	$(function () {
		let windowRange = {
			value: null
			...
			slide: (function (msStartDate, incrementFactor, windowRange) {
				return function (event, ui) {
					...
					if ($("#lock-window").prop("checked") && isInitalized(
						...
						if (ui.values[1] - ui.values[0] != windowRange
							.value) {
							let valueDelta = 0
							if (followingHandle == 1) {
								valueDelta = windowRange.value - (ui.values[
									1] - ui
										.values[0]);
							...
							if ((followingHandleTargetValue >= $(
								"#slider-range")
								.slider("option", "min")) && (
									followingHandleTargetValue <= $(
										"#slider-range")
										.slider("option", "max"))) {
								$("#slider-range").slider('values',
									followingHandle,
									ui.values[followingHandle] +
									valueDelta);
	...
	let hideGroupMarkers = function (toogleEnabled, group) {
		if (toogleEnabled) {
			...
			group.markers.forEach(function (element, index) {
				...
				if (isInitalized(group.elements[index].latitude) && isInitalized(group
					.elements[
					index].longitude)) {
					//addMarker(element);
	...
	if (isInitalized(exposureGroups)) {
		...
		for (const key of exposureGroupKeys) {
			...
			if (groupType == GROUP_TYPES.TRAVEL) {
				...
				let groupTimeBounds = (groupTimeBoundsEndTime - groupTimeBoundsStartTime) /
					incrementFactor;

				if (groupTimeBounds < 60) {
					if (groupTimeBounds < 30) {
						if (groupTimeBounds < 15) {
							hideGroupMarkers($("#hide-15").prop("checked") || $("#hide-30").prop(
								"checked") ||
								$(
									"#hide-60").prop("checked"), group);
						} else {
							hideGroupMarkers($("#hide-30").prop("checked") || $("#hide-60").prop(
								"checked"),
								group);
						}
					} else {
